...
generate_dockerfile() {
    ...
    # Build the command
    local cmd="./target/release/clippier generate-dockerfile . moosicbox_${package_name} --output $dockerfile_path --port $port --build-args STATIC_TOKEN,WS_HOST,TUNNEL_ACCESS_TOKEN"
    if [[ -n "$features" ]]; then
