...
mod tests {
    ...
    #[test]
    fn test_selector_try_from_valid() {
        // Test valid selector parsing
        assert_eq!(Selector::try_from("self").unwrap(), Selector::SelfTarget);
        assert_eq!(
            Selector::try_from("#my-id").unwrap(),
            Selector::Id("my-id".to_string())
        );
        assert_eq!(
            Selector::try_from(".my-class").unwrap(),
            Selector::Class("my-class".to_string())
        );
        assert_eq!(
            Selector::try_from("> .child-class").unwrap(),
            Selector::ChildClass("child-class".to_string())
        );
    }

    #[test]
    fn test_selector_try_from_invalid() {
        // Test invalid selector parsing
        assert!(Selector::try_from("invalid").is_err());
        assert!(Selector::try_from("@invalid").is_err());
    ...
    fn test_selector_display() {
        assert_eq!(Selector::SelfTarget.to_string(), "this");
        assert_eq!(Selector::Id("test-id".to_string()).to_string(), "#test-id");
        assert_eq!(
            Selector::Class("test-class".to_string()).to_string(),
            ".test-class"
        );
        assert_eq!(
            Selector::ChildClass("child".to_string()).to_string(),
            "> .child"
        );
    }

    #[test]
    fn test_selector_factory_methods() {
        assert_eq!(Selector::id("test"), Selector::Id("test".to_string()));
        assert_eq!(Selector::class("cls"), Selector::Class("cls".to_string()));
        assert_eq!(
            Selector::child_class("child"),
            Selector::ChildClass("child".to_string())
        );
    }

    #[test]
    fn test_target_as_str() {
        let literal = Target::Literal("test".to_string());
        let reference = Target::Ref("test".to_string());

        assert_eq!(literal.as_str(), Some("test"));
        assert_eq!(reference.as_str(), Some("test"));
    }

    #[test]
    fn test_target_factory_methods() {
        assert_eq!(Target::literal("test"), Target::Literal("test".to_string()));
        assert_eq!(Target::reference("ref"), Target::Ref("ref".to_string()));
    }

    #[test]
    fn test_element_target_factory_methods() {
        let target = Target::Literal("test".to_string());
        assert_eq!(
            ElementTarget::str_id(target.clone()),
            ElementTarget::StrId(target.clone())
        );
        assert_eq!(
            ElementTarget::class(target.clone()),
            ElementTarget::Class(target.clone())
        );
        assert_eq!(
            ElementTarget::child_class(target.clone()),
            ElementTarget::ChildClass(target)
        );
    }

    #[test]
    fn test_element_target_display() {
        let target = Target::Literal("test".to_string());

        assert_eq!(ElementTarget::StrId(target.clone()).to_string(), "#test");
        assert_eq!(ElementTarget::Class(target.clone()).to_string(), ".test");
        assert_eq!(ElementTarget::ChildClass(target).to_string(), "> .test");
        assert_eq!(ElementTarget::Id(42).to_string(), "[internal-id]");
        assert_eq!(ElementTarget::SelfTarget.to_string(), "this");
        assert_eq!(ElementTarget::LastChild.to_string(), ":last-child");
    }

    #[test]
    fn test_position_is_relative() {
        // Relative positions
        assert!(Position::Static.is_relative());
        assert!(Position::Relative.is_relative());
        assert!(Position::Sticky.is_relative());

        // Non-relative positions
    ...
    #[cfg(feature = "layout")]
    #[test]
    fn test_layout_position_row_column() {
        let wrap_pos = LayoutPosition::Wrap { row: 5, col: 10 };
        let default_pos = LayoutPosition::Default;

        assert_eq!(wrap_pos.row(), Some(5));
        assert_eq!(wrap_pos.column(), Some(10));

        assert_eq!(default_pos.row(), None);
        assert_eq!(default_pos.column(), None);
    }

    #[test]
    fn test_layout_direction_display() {
        assert_eq!(LayoutDirection::Row.to_string(), "row");
        assert_eq!(LayoutDirection::Column.to_string(), "col");
    }

    #[test]
    fn test_layout_overflow_display() {
        assert_eq!(LayoutOverflow::Auto.to_string(), "auto");
        assert_eq!(LayoutOverflow::Scroll.to_string(), "scroll");
        assert_eq!(LayoutOverflow::Expand.to_string(), "expand");
        assert_eq!(LayoutOverflow::Squash.to_string(), "squash");
        assert_eq!(LayoutOverflow::Wrap { grid: false }.to_string(), "wrap");
        assert_eq!(LayoutOverflow::Wrap { grid: true }.to_string(), "wrap-grid");
        assert_eq!(LayoutOverflow::Hidden.to_string(), "hidden");
    }

    #[test]
    fn test_justify_content_display() {
        assert_eq!(JustifyContent::Start.to_string(), "start");
        assert_eq!(JustifyContent::Center.to_string(), "center");
        assert_eq!(JustifyContent::End.to_string(), "end");
        assert_eq!(JustifyContent::SpaceBetween.to_string(), "space-between");
        assert_eq!(JustifyContent::SpaceEvenly.to_string(), "space-evenly");
    }

    #[test]
    fn test_align_items_display() {
        assert_eq!(AlignItems::Start.to_string(), "start");
        assert_eq!(AlignItems::Center.to_string(), "center");
        assert_eq!(AlignItems::End.to_string(), "end");
    }

    #[test]
    fn test_text_align_display() {
        assert_eq!(TextAlign::Start.to_string(), "start");
        assert_eq!(TextAlign::Center.to_string(), "center");
        assert_eq!(TextAlign::End.to_string(), "end");
        assert_eq!(TextAlign::Justify.to_string(), "justify");
    }

    #[test]
    fn test_swap_strategy_display() {
        assert_eq!(SwapStrategy::This.to_string(), "this");
        assert_eq!(SwapStrategy::Children.to_string(), "children");
        assert_eq!(SwapStrategy::BeforeBegin.to_string(), "beforebegin");
        assert_eq!(SwapStrategy::AfterBegin.to_string(), "afterbegin");
        assert_eq!(SwapStrategy::BeforeEnd.to_string(), "beforeend");
        assert_eq!(SwapStrategy::AfterEnd.to_string(), "afterend");
        assert_eq!(SwapStrategy::Delete.to_string(), "delete");
        assert_eq!(SwapStrategy::None.to_string(), "none");
    }

    #[test]
    fn test_cursor_display() {
        assert_eq!(Cursor::Auto.to_string(), "auto");
        assert_eq!(Cursor::Pointer.to_string(), "pointer");
        assert_eq!(Cursor::Text.to_string(), "text");
        assert_eq!(Cursor::Crosshair.to_string(), "crosshair");
        assert_eq!(Cursor::Move.to_string(), "move");
        assert_eq!(Cursor::NotAllowed.to_string(), "not-allowed");
        assert_eq!(Cursor::Grab.to_string(), "grab");
        assert_eq!(Cursor::Grabbing.to_string(), "grabbing");
        assert_eq!(Cursor::ZoomIn.to_string(), "zoom-in");
        assert_eq!(Cursor::ZoomOut.to_string(), "zoom-out");
    }

    #[test]
    fn test_position_display() {
        assert_eq!(Position::Static.to_string(), "static");
        assert_eq!(Position::Relative.to_string(), "relative");
        assert_eq!(Position::Absolute.to_string(), "absolute");
        assert_eq!(Position::Sticky.to_string(), "sticky");
        assert_eq!(Position::Fixed.to_string(), "fixed");
    }
    ...
    #[test]
    fn test_image_loading_display() {
        assert_eq!(ImageLoading::Eager.to_string(), "eager");
        assert_eq!(ImageLoading::Lazy.to_string(), "lazy");
    }

    #[test]
    fn test_image_fit_display() {
        assert_eq!(ImageFit::Default.to_string(), "default");
        assert_eq!(ImageFit::Contain.to_string(), "contain");
        assert_eq!(ImageFit::Cover.to_string(), "cover");
        assert_eq!(ImageFit::Fill.to_string(), "fill");
        assert_eq!(ImageFit::None.to_string(), "none");
    }

    #[test]
    fn test_text_decoration_line_display() {
        assert_eq!(TextDecorationLine::Inherit.to_string(), "inherit");
        assert_eq!(TextDecorationLine::None.to_string(), "none");
        assert_eq!(TextDecorationLine::Underline.to_string(), "underline");
        assert_eq!(TextDecorationLine::Overline.to_string(), "overline");
        assert_eq!(TextDecorationLine::LineThrough.to_string(), "line-through");
    }

    #[test]
    fn test_text_decoration_style_display() {
        assert_eq!(TextDecorationStyle::Inherit.to_string(), "inherit");
        assert_eq!(TextDecorationStyle::Solid.to_string(), "solid");
        assert_eq!(TextDecorationStyle::Double.to_string(), "double");
        assert_eq!(TextDecorationStyle::Dotted.to_string(), "dotted");
        assert_eq!(TextDecorationStyle::Dashed.to_string(), "dashed");
        assert_eq!(TextDecorationStyle::Wavy.to_string(), "wavy");
    }

    #[test]
    fn test_link_target_display() {
        assert_eq!(LinkTarget::SelfTarget.to_string(), "_self");
        ...
        assert_eq!(
            LinkTarget::Custom("custom".to_string()).to_string(),
            "custom"
        );
    }
    ...
    fn test_default_values() {
        assert_eq!(LayoutDirection::default(), LayoutDirection::Column);
        assert_eq!(LayoutOverflow::default(), LayoutOverflow::Expand);
        ...
        assert_eq!(Selector::default(), Selector::SelfTarget);
        assert_eq!(ElementTarget::default(), ElementTarget::SelfTarget);
        assert_eq!(SwapStrategy::default(), SwapStrategy::This);
        assert_eq!(Cursor::default(), Cursor::Auto);
        assert_eq!(Position::default(), Position::Static);
        assert_eq!(Visibility::default(), Visibility::Visible);
        assert_eq!(ImageLoading::default(), ImageLoading::Eager);
        assert_eq!(ImageFit::default(), ImageFit::Default);
        assert_eq!(TextDecorationLine::default(), TextDecorationLine::Inherit);
        assert_eq!(TextDecorationStyle::default(), TextDecorationStyle::Inherit);
        assert_eq!(LinkTarget::default(), LinkTarget::SelfTarget);
    }

    #[cfg(feature = "layout")]
    #[test]
    fn test_layout_position_default() {
        assert_eq!(LayoutPosition::default(), LayoutPosition::Default);
    }
}
