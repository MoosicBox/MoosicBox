...
fn render_impl() {
    struct R(&'static str);
    impl hyperchad_template::Render for R {
...
#[test]
fn issue_97() {
    use hyperchad_template::Render;

    struct Pinkie;
    impl Render for Pinkie {
        fn render(&self) -> hyperchad_template::Markup {
            let x = 42;
            html! { (x) }
        }
    }
...
fn prefer_render_over_display() {
    ...
    assert_eq!(
        html! { (hyperchad_template::display(RenderAndDisplay)) }.into_string(),
        "&lt;display&gt;"
    );
}
...
#[test]
fn default() {
    use hyperchad_template::{Markup, PreEscaped};
