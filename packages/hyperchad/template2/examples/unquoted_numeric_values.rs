...
fn main() {
    ...
    println!("{}\n", plain_numbers.iter().map(|c| c.to_string()).collect::<String>());
    
    println!("✅ PERCENTAGE VALUES:");
    println!("{}\n", percentage_values.iter().map(|c| c.to_string()).collect::<String>());
    
    println!("✅ MIXED VALUES:");
    println!("{}\n", mixed_values.iter().map(|c| c.to_string()).collect::<String>());
    
    println!("✅ DECIMAL VALUES:");
    println!("{}\n", decimal_values.iter().map(|c| c.to_string()).collect::<String>());
    
    println!("⚠️ VIEWPORT UNITS (quoted):");
    println!("{}\n", viewport_units.iter().map(|c| c.to_string()).collect::<String>());
