...
mod tests {
    ...
    fn test_api_download_location_to_value_type_from_json() {
        ...
        let result: Result<ApiDownloadLocation, ParseError> = (&json).to_value_type();
    ...
    #[test_log::test]
    fn test_api_download_task_state_to_value_type_from_json_valid() {
        let states = [
            ("PENDING", ApiDownloadTaskState::Pending),
            ("PAUSED", ApiDownloadTaskState::Paused),
            ("CANCELLED", ApiDownloadTaskState::Cancelled),
            ("STARTED", ApiDownloadTaskState::Started),
            ("FINISHED", ApiDownloadTaskState::Finished),
            ("ERROR", ApiDownloadTaskState::Error),
        ];
    ...
    #[test_log::test]
    fn test_stripped_api_download_item_to_value_type_from_json_track() {
        let source_json =
    ...
    #[test_log::test]
    fn test_stripped_api_download_item_to_value_type_from_json_album_cover() {
        let json = serde_json::json!({
            "type": "ALBUM_COVER",
            "album_id": 456
        });
    ...
    #[test_log::test]
    fn test_stripped_api_download_item_to_value_type_from_json_artist_cover() {
        let json = serde_json::json!({
            "type": "ARTIST_COVER",
            "album_id": 789
        });
    ...
    #[test_log::test]
    fn test_api_download_item_to_value_type_from_json_track() {
        let source_json =
            serde_json::to_value(DownloadApiSource::Api(TEST_API_SOURCE.clone())).unwrap();
        let json = serde_json::json!({
            "type": "TRACK",
    ...
    #[test_log::test]
    fn test_api_download_item_to_value_type_from_json_album_cover() {
        let source_json =
            serde_json::to_value(DownloadApiSource::Api(TEST_API_SOURCE.clone())).unwrap();
        let json = serde_json::json!({
            "type": "ALBUM_COVER",
    ...
    #[test_log::test]
    fn test_api_download_item_to_value_type_from_json_artist_cover() {
        let source_json =
            serde_json::to_value(DownloadApiSource::Api(TEST_API_SOURCE.clone())).unwrap();
        let json = serde_json::json!({
            "type": "ARTIST_COVER",
    ...
    fn test_api_download_task_to_value_type_from_json() {
        ...
        let result: Result<ApiDownloadTask, ParseError> = (&json).to_value_type();
        let task = result.unwrap();

        assert_eq!(task.id, 99);
        assert_eq!(task.state, ApiDownloadTaskState::Started);
        assert_eq!(task.file_path, "/music/test.flac");
        assert_eq!(task.total_bytes, Some(2_048_000));
        // speed should be None (not in JSON)
        assert_eq!(task.speed, None);
    }
