...
mod tests {
    ...
    #[test]
    fn test_new_profile_form_with_message_and_value() {
        let result = new_profile_form(
        ...
        assert!(html.contains("test-profile"));
        assert!(html.contains("bundled=false"));
    }

    #[test]
    fn test_new_profile_form_bundled_uses_realname() {
        let result = new_profile_form(None, None, true);
        let html = result.into_string();

        assert!(html.contains("bundled=true"));
        // Should contain the real name (from whoami::realname())
        assert!(html.contains("value="));
    }

    #[test]
    fn test_new_profile_form_not_bundled_empty_value() {
        let result = new_profile_form(None, None, false);
        let html = result.into_string();
    ...
    #[test]
    fn test_select_form_with_trigger() {
        let profiles = vec!["profile1", "profile2"];
        let result = select_form(&profiles, Some("profile1"), Some("custom-event from:body"));
        let html = result.into_string();

        assert!(html.contains("hx-post=\"/admin/profiles/select\""));
        assert!(html.contains("hx-trigger=\"change, custom-event from:body\""));
    }

    #[test]
    fn test_select_form_without_trigger() {
        let profiles = vec!["profile1", "profile2"];
        let result = select_form(&profiles, Some("profile1"), None);
        let html = result.into_string();
    ...
    #[test]
    fn test_select_with_selected_profile() {
        let profiles = vec!["profile1", "profile2", "profile3"];
        let result = select(&profiles, Some("profile2"));
        let html = result.into_string();

        assert!(html.contains("profile1"));
        assert!(html.contains("profile2"));
        assert!(html.contains("profile3"));
        // The selected profile should have the 'selected' attribute
        assert!(html.contains("selected"));
    }

    #[test]
    fn test_select_without_selected_profile() {
        let profiles = vec!["profile1", "profile2"];
        let result = select(&profiles, None);
        let html = result.into_string();

        assert!(html.contains("profile1"));
        assert!(html.contains("profile2"));
        // No profile should be selected
        assert!(!html.contains("selected"));
    }

    #[test]
    fn test_select_with_empty_profiles() {
        let profiles: Vec<&str> = vec![];
        let result = select(&profiles, None);
        let html = result.into_string();

        // Should render an empty select element
        assert!(html.contains("<select"));
        assert!(html.contains("</select>"));
    }

    #[test]
    fn test_profile_renders_delete_button() {
        let result = profile("my-profile");
        let html = result.into_string();

        assert!(html.contains("my-profile"));
        assert!(html.contains("hx-delete=\"/admin/profiles\""));
