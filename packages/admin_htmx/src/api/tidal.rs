...
mod tests {
    ...
    fn test_settings_logged_in_with_scan() {
        ...
        assert!(html.contains("hx-post=\"/admin/tidal/run-scan\""));
    }
    ...
    #[test]
    fn test_settings_logged_out() {
        let result = settings_logged_out();
        let html = result.into_string();

        assert!(html.contains("hx-post=\"/admin/tidal/auth/device-authorization\""));
        assert!(html.contains("Start web authentication"));
    }
}
