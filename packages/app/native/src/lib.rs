...
pub async fn convert_state(app_state: &moosicbox_app_state::AppState) -> state::State {
    ...
    state.connection_status = app_state.connection_status().await;
    ...
    let router =
        Router::new()
            ...
            .with_route_result("/downloads", |req| async {
                routes::handle_route_result(routes::downloads_route(req).await)
            })
            .with_route_result("/settings", |req| async {
                routes::handle_route_result(routes::settings_route(req).await)
            })
            .with_route_result("/settings/connections", |req| async {
                routes::handle_route_result(routes::settings_connections_route(req).await)
            })
            .with_route_result("/settings/new-connection", |req| async {
                routes::handle_route_result(routes::settings_new_connection_route(req).await)
            })
            .with_route_result("/settings/select-connection", |req| async {
                routes::handle_route_result(routes::settings_select_connection_route(req).await)
            })
            ...
            .with_route_result("/settings/music-api-settings", |req| async {
                routes::handle_route_result(routes::settings_music_api_settings_route(req).await)
            })
            .with_route_result("/settings/download-settings", |req| async {
                routes::handle_route_result(routes::settings_download_settings_route(req).await)
            })
            .with_route_result("/settings/downloads/download-location", |req| async move {
                routes::handle_route_result(
            ...
            .with_route_result("/settings/scan-settings", |req| async {
                routes::handle_route_result(routes::settings_scan_settings_route(req).await)
            })
            .with_route_result("/settings/scan/scan-path", |req| async move {
                routes::handle_route_result(routes::settings_scan_scan_path_route(req).await)
            })
            .with_route_result("/audio-zones", |req| async {
                routes::handle_route_result(routes::audio_zones_route(req).await)
            })
            .with_route_result("/playback-sessions", |req| async {
                routes::handle_route_result(routes::playback_sessions_route(req).await)
            })
            ...
            .with_route_result("/albums-list-start", |req| async move {
                routes::handle_route_result(routes::albums_list_start_route(req).await)
            })
            .with_route_result("/albums-list", |req| async move {
                routes::handle_route_result(routes::albums_list_route(req).await)
            })
            .with_route_result(
                "/artists",
            ...
            .with_route_result("/artists/albums-list", |req| async move {
                routes::handle_route_result(routes::artist_albums_list_route(req).await)
            })
            .with_route_result("/music-api/scan", |req| async move {
                routes::handle_route_result(routes::music_api_scan_route(req).await)
            })
            .with_route_result("/music-api/enable-scan-origin", |req| async move {
                routes::handle_route_result(routes::music_api_enable_scan_origin_route(req).await)
            })
            .with_route_result("/music-api/auth", |req| async move {
                routes::handle_route_result(routes::music_api_auth_route(req).await)
            })
            ...
            .with_route_result("/library", |req| async move {
                routes::handle_route_result(routes::library_route(req).await)
            });
