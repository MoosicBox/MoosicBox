...
mod tests {
    ...
    mod download_tab_tests {
        use super::*;

        #[test_log::test]
        fn test_display_current_tab() {
            assert_eq!(DownloadTab::Current.to_string(), "CURRENT");
        }

        #[test_log::test]
        fn test_display_history_tab() {
            assert_eq!(DownloadTab::History.to_string(), "HISTORY");
        }

        #[test_log::test]
        fn test_from_str_current() {
            assert_eq!(
                DownloadTab::from_str("CURRENT").unwrap(),
                DownloadTab::Current
            );
        }

        #[test_log::test]
        fn test_from_str_history() {
            assert_eq!(
                DownloadTab::from_str("HISTORY").unwrap(),
                DownloadTab::History
            );
        }
        ...
        fn test_from_str_invalid() {
            assert!(DownloadTab::from_str("invalid").is_err());
        }

        #[test_log::test]
        fn test_equality() {
            assert_eq!(DownloadTab::Current, DownloadTab::Current);
            assert_eq!(DownloadTab::History, DownloadTab::History);
            assert_ne!(DownloadTab::Current, DownloadTab::History);
        }
    }
}
