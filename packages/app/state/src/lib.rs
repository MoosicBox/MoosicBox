...
mod tests {
    ...
    fn test_update_app_state_default() {
        ...
        assert!(update.connection_name.is_none());
        assert!(update.api_url.is_none());
        assert!(update.client_id.is_none());
        assert!(update.signature_token.is_none());
        assert!(update.api_token.is_none());
        assert!(update.profile.is_none());
        assert!(update.playback_target.is_none());
    ...
    fn test_update_app_state_display() {
        let update = UpdateAppState {
            ...
            connection_name: Some(Some("Test Connection".to_string())),
            api_url: Some(Some("https://api.test.com".to_string())),
            ..Default::default()
        };

        let display_str = format!("{update}");
        assert!(display_str.contains("connection_id"));
    ...
    fn test_update_app_state_serialization() {
        let update = UpdateAppState {
            ...
            api_url: Some(Some("https://example.com".to_string())),
            profile: Some(Some("master".to_string())),
            ..Default::default()
        };

        let json = serde_json::to_string(&update).unwrap();
        let deserialized: UpdateAppState = serde_json::from_str(&json).unwrap();

        assert_eq!(deserialized.connection_id, update.connection_id);
        assert_eq!(deserialized.api_url, update.api_url);
    ...
    fn test_player_type_debug_local() {
        ...
        assert_eq!(debug_str, "Local");
    }

    #[test_log::test(switchy_async::test)]
    async fn test_app_state_get_default_download_location_none() {
        let state = AppState::new();
        let location = state.get_default_download_location();
        assert!(location.is_none());
    }

    #[test_log::test(switchy_async::test)]
    async fn test_app_state_builder_pattern_with_listeners() {
        let state = AppState::new()
            .with_on_before_handle_playback_update_listener(|_update| async {
                // Listener callback
            })
            .with_on_after_handle_playback_update_listener(|_update| async {
                // Listener callback
            })
            .with_on_before_update_playlist_listener(|| async {
                // Listener callback
            })
            .with_on_after_update_playlist_listener(|_session| async {
                // Listener callback
            });

        assert_eq!(state.on_before_handle_playback_update_listeners.len(), 1);
        assert_eq!(state.on_after_handle_playback_update_listeners.len(), 1);
        assert_eq!(state.on_before_update_playlist_listeners.len(), 1);
        assert_eq!(state.on_after_update_playlist_listeners.len(), 1);
    }

    #[test_log::test(switchy_async::test)]
    async fn test_app_state_builder_pattern_with_message_listeners() {
        let state = AppState::new()
            .with_on_before_handle_ws_message_listener(|_message| async {
                // Listener callback
            })
            .with_on_after_handle_ws_message_listener(|_message| async {
                // Listener callback
            })
            .with_on_before_set_state_listener(|_state| async {
                // Listener callback
            })
            .with_on_after_set_state_listener(|_state| async {
                // Listener callback
            });

        assert_eq!(state.on_before_handle_ws_message_listeners.len(), 1);
        assert_eq!(state.on_after_handle_ws_message_listeners.len(), 1);
        assert_eq!(state.on_before_set_state_listeners.len(), 1);
        assert_eq!(state.on_after_set_state_listeners.len(), 1);
    }

    #[test_log::test(switchy_async::test)]
    async fn test_app_state_builder_pattern_with_update_listeners() {
        let state = AppState::new()
            .with_on_current_sessions_updated_listener(|_sessions| async {
                // Listener callback
            })
            .with_on_audio_zone_with_sessions_updated_listener(|_zones| async {
                // Listener callback
            })
            .with_on_connections_updated_listener(|_connections| async {
                // Listener callback
            });

        assert_eq!(state.on_current_sessions_updated_listeners.len(), 1);
        assert_eq!(state.on_audio_zone_with_sessions_updated_listeners.len(), 1);
        assert_eq!(state.on_connections_updated_listeners.len(), 1);
    }
}
