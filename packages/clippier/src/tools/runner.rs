...
impl<'a> ToolRunner<'a> {
    ...
    fn terminate_process_tree(child: &mut std::process::Child) {
        // Use OS utilities without a shell; the ID is generated by spawn, never input.
        #[cfg(unix)]
        ...
        if !result.is_ok_and(|status| status.success()) {
            log::warn!("Could not terminate the entire tool process tree; killing direct child");
        }
        let _ = child.kill();
        let _ = child.wait();
    }
...
mod tests {
    ...
    #[test]
    #[cfg(unix)]
    fn cancellation_closes_descendant_pipes_even_after_parent_exits() {
        for script in ["sleep 3 & wait", "sleep 3 & exit 0"] {
            let start = Instant::now();
            let mut command = Command::new("sh");
