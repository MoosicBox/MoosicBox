...
#[test]
fn test_packages_filter_performance() {
    // Test that filtering is actually faster than processing all packages
    let (temp_dir, _) = load_test_workspace("complex");

    let start_all = std::time::Instant::now();
    let _ = handle_features_command(
        temp_dir.path().to_str().unwrap(),
        None,
        None,
        None,
    ...
    let start_filtered = std::time::Instant::now();
    let _ = handle_features_command(
        temp_dir.path().to_str().unwrap(),
        None,
        None,
        ...
        None,
        None,
        Some(&["api".to_string()]), // Process only one package
        None,
        #[cfg(feature = "git-diff")]
        None,
        #[cfg(feature = "git-diff")]
    ...
    // Filtered should be significantly faster (or at least not slower)
    assert!(duration_filtered <= duration_all);
}
