...
mod tests {
    ...
    use super::{TestArgs, TestWithPathInput, build_test_tokens};

    /// Tests parsing of empty `TestArgs`.
    #[::core::prelude::v1::test]
    fn test_args_empty() {
        let input = quote! {};
        ...
        assert!(!args.real_fs);
        assert!(!args.no_simulator);
    }

    /// Tests parsing of `TestArgs` with only `real_time` flag.
    #[::core::prelude::v1::test]
    fn test_args_real_time() {
    ...
    /// Tests parsing of `TestArgs` with only `real_fs` flag.
    #[::core::prelude::v1::test]
    fn test_args_real_fs() {
    ...
    /// Tests parsing of `TestArgs` with only `no_simulator` flag.
    #[::core::prelude::v1::test]
    fn test_args_no_simulator() {
    ...
    /// Tests parsing of `TestArgs` with `real_time` and `real_fs` flags.
    #[::core::prelude::v1::test]
    fn test_args_real_time_and_real_fs() {
        let input = quote! { real_time, real_fs };
        let args: TestArgs =
            syn::parse2(input).expect("Failed to parse TestArgs with real_time and real_fs");
        assert!(args.real_time);
        assert!(args.real_fs);
        assert!(!args.no_simulator);
    }

    /// Tests parsing of `TestArgs` with all flags.
    #[::core::prelude::v1::test]
    ...
    /// Tests parsing of `TestArgs` with flags in different order.
    #[::core::prelude::v1::test]
    fn test_args_different_order() {
        let input = quote! { no_simulator, real_fs, real_time };
        let args: TestArgs =
            syn::parse2(input).expect("Failed to parse TestArgs with flags in different order");
        assert!(args.real_time);
        assert!(args.real_fs);
        assert!(args.no_simulator);
    }

    /// Tests parsing of `TestArgs` with an invalid flag returns an error.
    #[::core::prelude::v1::test]
    fn test_args_invalid_flag() {
        let input = quote! { invalid_flag };
        let result = syn::parse2::<TestArgs>(input);
        assert!(
            result.is_err(),
            "Parsing TestArgs with invalid flag should fail"
        );
    }

    /// Tests parsing of `TestArgs` with a mix of valid and invalid flags.
    #[::core::prelude::v1::test]
    fn test_args_mixed_valid_invalid() {
        let input = quote! { real_time, invalid_flag };
        let result = syn::parse2::<TestArgs>(input);
        assert!(
            result.is_err(),
            "Parsing TestArgs with mixed valid/invalid flags should fail"
        );
    }

    /// Tests `build_test_tokens` with default crate path and no flags.
    #[::core::prelude::v1::test]
    fn build_test_tokens_default() {
        let args = TestArgs {
            real_time: false,
            real_fs: false,
            no_simulator: false,
        };
        let item_tokens = quote! { async fn test_fn() {} };

        let result = build_test_tokens("crate", &args, &item_tokens);
        let result_str = result.to_string();

        // Check that the tokens contain the expected elements
        assert!(
        ...
        assert!(
            !result_str.contains("real_time"),
            "Should not contain real_time"
        );
        assert!(
            !result_str.contains("real_fs"),
            "Should not contain real_fs"
        );
        assert!(
            !result_str.contains("no_simulator"),
            "Should not contain no_simulator"
        );
    }

    /// Tests `build_test_tokens` with `real_time` flag.
    #[::core::prelude::v1::test]
    fn build_test_tokens_with_real_time() {
        ...
        let result = build_test_tokens("switchy_async", &args, &item_tokens);
        let result_str = result.to_string();

        assert!(
            result_str.contains("real_time"),
            "Should contain real_time flag"
        );
        assert!(
            !result_str.contains("real_fs"),
            "Should not contain real_fs"
    ...
    /// Tests `build_test_tokens` with `real_fs` flag.
    #[::core::prelude::v1::test]
    ...
    /// Tests `build_test_tokens` with `no_simulator` flag.
    #[::core::prelude::v1::test]
    fn build_test_tokens_with_no_simulator() {
        ...
        assert!(
            result_str.contains("no_simulator"),
            "Should contain no_simulator flag"
        );
    }

    /// Tests `build_test_tokens` with all flags enabled.
    #[::core::prelude::v1::test]
    fn build_test_tokens_all_flags() {
        ...
        let result = build_test_tokens("switchy::unsync", &args, &item_tokens);
        let result_str = result.to_string();

        assert!(result_str.contains("real_time"), "Should contain real_time");
        assert!(result_str.contains("real_fs"), "Should contain real_fs");
        assert!(
            result_str.contains("no_simulator"),
            "Should contain no_simulator"
        );
    }

    /// Tests `build_test_tokens` with custom crate path.
    #[::core::prelude::v1::test]
    fn build_test_tokens_custom_path() {
        let args = TestArgs {
            real_time: false,
            real_fs: false,
            no_simulator: false,
        ...
        let result = build_test_tokens("my_custom::path", &args, &item_tokens);
        let result_str = result.to_string();

        assert!(
            result_str.contains("my_custom :: path"),
            "Should contain custom path"
        );
    }

    /// Tests parsing of `TestWithPathInput` with basic configuration.
    #[::core::prelude::v1::test]
    fn test_with_path_input_basic() {
        ...
        let parsed: TestWithPathInput =
            syn::parse2(input).expect("Failed to parse TestWithPathInput");

        assert_eq!(
            parsed.crate_path,
            parse_quote!(crate),
            "Crate path should be 'crate'"
        );
        assert!(!parsed.use_real_time, "Should not use real time");
        assert!(!parsed.use_real_fs, "Should not use real fs");
        assert!(!parsed.no_simulator, "Should not disable simulator");
        assert_eq!(
            parsed.function.sig.ident.to_string(),
            "test_fn",
            "Function name should be 'test_fn'"
        );
    }
    ...
    fn test_with_path_input_real_time() {
        ...
        assert!(parsed.use_real_time, "Should use real time");
        assert!(!parsed.use_real_fs, "Should not use real fs");
        assert!(!parsed.no_simulator, "Should not disable simulator");
    }
    ...
    fn test_with_path_input_real_fs() {
        ...
        assert!(!parsed.use_real_time, "Should not use real time");
        assert!(parsed.use_real_fs, "Should use real fs");
        assert!(!parsed.no_simulator, "Should not disable simulator");
    }
    ...
    fn test_with_path_input_no_simulator() {
        ...
        assert!(!parsed.use_real_time, "Should not use real time");
        assert!(!parsed.use_real_fs, "Should not use real fs");
        assert!(parsed.no_simulator, "Should disable simulator");
    }

    /// Tests parsing of `TestWithPathInput` with all flags.
    #[::core::prelude::v1::test]
    fn test_with_path_input_all_flags() {
        let input = quote! {
    ...
    /// Tests parsing of `TestWithPathInput` with synchronous function.
    #[::core::prelude::v1::test]
