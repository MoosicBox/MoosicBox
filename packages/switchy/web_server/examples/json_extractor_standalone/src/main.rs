...
//!
//! This example shows how to use the `Json<T>` extractor from `switchy_web_server`
...
#[cfg(any(feature = "actix", feature = "simulator"))]
use switchy_web_server::{Error, HttpResponse, Json, RequestData};
...
fn run_actix_examples() {
    ...
    let routes = [
        switchy_web_server::Route::with_handler1(
            switchy_web_server::Method::Post,
            "/simple",
            simple_json_handler,
        ),
        switchy_web_server::Route::with_handler1(
            switchy_web_server::Method::Patch,
            "/optional",
...
fn run_simulator_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Post, "/simple")
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Patch, "/optional")
        .with_header("content-type", "application/json")
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Post, "/echo")
        .with_header("content-type", "application/json")
        .with_body(user_json.as_bytes().to_vec());
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Post, "/error")
