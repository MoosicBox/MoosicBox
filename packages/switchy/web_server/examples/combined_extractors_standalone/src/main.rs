...
//! real-world API endpoints with the `switchy_web_server` framework.
//!
//! # Features
//!
...
#[cfg(any(feature = "actix", feature = "simulator"))]
use switchy_web_server::{Error, HttpResponse, Json, Query, RequestData};
...
fn run_simulator_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Get, "/json-info")
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Get, "/double")
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Get, "/search")
