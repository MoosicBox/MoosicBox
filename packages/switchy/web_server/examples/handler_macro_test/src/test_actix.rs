...
use switchy_web_server::{Headers, HttpResponse, Method, Path, Query, RequestInfo, Route};
...
async fn simple_handler() -> Result<HttpResponse, switchy_web_server::Error> {
...
///
/// Returns an error if the response cannot be created.
async fn info_handler(info: RequestInfo) -> Result<HttpResponse, switchy_web_server::Error> {
...
///
/// Returns an error if the response cannot be created.
async fn headers_handler(headers: Headers) -> Result<HttpResponse, switchy_web_server::Error> {
...
) -> Result<HttpResponse, switchy_web_server::Error> {
...
///
/// Returns an error if the response cannot be created.
async fn path_handler(Path(user_id): Path<u32>) -> Result<HttpResponse, switchy_web_server::Error> {
