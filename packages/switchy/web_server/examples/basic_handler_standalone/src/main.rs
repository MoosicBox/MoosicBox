...
//! the `RequestData` extractor from `switchy_web_server`, without requiring
...
use switchy_web_server::{Error, HttpResponse, RequestData};
...
fn run_simulator_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = SimulationRequest::new(switchy_web_server::Method::Get, "/basic-info")
