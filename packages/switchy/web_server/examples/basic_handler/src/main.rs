...
use switchy_web_server::{Error, HttpResponse, Method, RequestData, Route};
#[cfg(any(feature = "simulator", not(feature = "actix")))]
use switchy_web_server::{FromRequest, HttpRequest};
...
#[cfg_attr(
    all(
        feature = "actix",
        not(any(feature = "simulator", not(feature = "actix")))
    ),
