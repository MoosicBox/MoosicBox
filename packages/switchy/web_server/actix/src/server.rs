...
use switchy_web_server::{HttpResponse, HttpResponseBody, Method, Scope, request::HttpRequest};
...
pub trait WebServerBuilderActixExt {
    ...
    fn build_actix_with_static(
        self,
        static_files: Vec<switchy_web_server::StaticFiles>,
    ) -> Box<dyn WebServer>;
}

impl WebServerBuilderActixExt for switchy_web_server::WebServerBuilder {
...
fn build_actix_server(
    builder: &switchy_web_server::WebServerBuilder,
    #[allow(unused_variables)] additional_static_files: &[switchy_web_server::StaticFiles],
) -> Box<dyn WebServer> {
