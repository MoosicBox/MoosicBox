...
///
/// This extractor parses path parameters from the URL path.
/// For now, it's a simple wrapper that can be extended with proper path parsing.
///
/// # Example
///
/// ```rust,ignore
...
///
/// async fn get_user(Path(user_id): Path<u64>) -> Result<HttpResponse, Box<dyn std::error::Error>> {
///     // user_id extracted from path like "/users/123"
///     Ok(HttpResponse::ok())
/// }
/// ```
#[derive(Debug, Clone)]
    ...
    T: DeserializeOwned + Send + 'static,
{
    ...
    fn from_request_sync(req: &HttpRequest) -> Result<Self, Self::Error> {
        ...
        // Extract the last path segment and try to deserialize it as type T
        let segments: Vec<&str> = path.split('/').filter(|s| !s.is_empty()).collect();

        if let Some(last_segment) = segments.last() {
            // Try to parse the last segment as a string first, then deserialize
            // This works for simple types like String, u32, i32, etc.

            // For String type, just return the segment directly
            let json_str = format!("\"{last_segment}\"");
            match serde_json::from_str::<T>(&json_str) {
                Ok(value) => return Ok(Self(value)),
                Err(_) => {
                    // If string parsing fails, try parsing as a number
                    if let Ok(value) = serde_json::from_str::<T>(last_segment) {
        ...
        // If we can't extract from the last segment, return an error with helpful message
        Err(Error::bad_request(format!(
            "Could not extract path parameter of type '{}' from path '{}'. Expected a single path segment that can be parsed as the target type.",
            std::any::type_name::<T>(),
            path
        )))
    }
