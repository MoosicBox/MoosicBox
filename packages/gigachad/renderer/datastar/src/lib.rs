...
impl DatastarRenderer {
    ...
    pub fn new(router: Router, runtime: Arc<Runtime>, request_action: Sender<String>) -> Self {
        ...
        Self {
            ...
            x: None,
            y: None,
            app: DatastarApp::new(router, request_action),
            receiver: rx,
            runtime,
        }
    }
...
pub async fn catchall_endpoint(
    ...
    Ok(HttpResponse::Ok()
        ...
        .body(
            container_element_to_html_response(&container.immediate, app.background)
                .map_err(ErrorInternalServerError)?,
        ))
}

pub struct DatastarRenderRunner {
    app: DatastarApp,
    handle: Handle,
}

impl RenderRunner for DatastarRenderRunner {
    /// # Errors
    ///
...
impl Renderer for DatastarRenderer {
    ...
    async fn to_runner(
        ...
        Ok(Box::new(DatastarRenderRunner {
            app: self.app.clone(),
