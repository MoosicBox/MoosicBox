-- Rustdoc Library Filter Transform
-- Filters out packages that don't have src/lib.rs or src/main.rs
--
-- Problem: The rustdoc-checker workflow needs to document public APIs,
...
function transform(context, matrix)
  ...
  for _, entry in ipairs(matrix) do
    ...
    if not package then
      ...
      if has_lib or has_main then
        -- Package has documentable source code
        table.insert(result, entry)
      else
        -- Package has no lib.rs or main.rs, filter it out
        filtered_count = filtered_count + 1
        context:warn(
          string.format("Filtering %s from rustdoc matrix (no src/lib.rs or src/main.rs found)", package)
        )
      end
    end
  end
  ...
  if filtered_count > 0 then
    context:log(string.format("Filtered %d package(s) without documentable source code", filtered_count))
  else
    context:log("All packages have documentable source code")
  end
