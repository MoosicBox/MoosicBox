-- ASIO compatibility transform for CI
-- Removes features that transitively enable cpal/asio on Windows
--
-- Problem: The ASIO SDK is not available on Windows CI runners, so any build
-- that enables the cpal/asio feature will fail. This transform walks the
-- dependency graph to find and remove features that would transitively
-- activate cpal/asio on Windows builds.
--
-- This is a real-world example of dependency analysis that cannot be expressed
-- in static configuration files like clippier.toml.
