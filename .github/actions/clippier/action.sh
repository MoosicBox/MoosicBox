...
find_valid_commit_from_api() {
    ...
    if [[ -z "$workflow_name" && -n "$GITHUB_WORKFLOW_REF" ]]; then
        workflow_name=$(basename "$GITHUB_WORKFLOW_REF" | sed 's/@.*//')
    fi
    ...
    local workflow_runs=$(curl -s \
        ...
        "https://api.github.com/repos/${INPUT_GITHUB_REPOSITORY}/actions/workflows/${workflow_name}/runs?branch=${INPUT_GITHUB_REF_NAME}&per_page=10")

    for i in $(seq 0 9); do
        local run_sha=$(printf '%s' "$workflow_runs" | jq -r ".workflow_runs[$i].head_sha // empty")

        if [[ -z "$run_sha" || "$run_sha" == "null" || "$run_sha" == "empty" ]]; then
            echo "No more workflow runs to check (checked $i runs)" >&2
            break
        fi
        ...
        echo "Checking commit from workflow history: $run_sha" >&2
        if git cat-file -e "$run_sha" 2>/dev/null; then
            echo "Found valid commit: $run_sha" >&2
            echo "$run_sha"
            return
        fi
    done

    echo "No valid commits found in workflow history" >&2
    echo "origin/${INPUT_GIT_DEFAULT_BRANCH}"
}
